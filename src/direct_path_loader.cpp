...
void SetTextAttr(void *handle, ub4 handle_type, const std::string &value, ub4 attribute, OCIError *err,
                 const char *what) {
	CheckOci(Oci().OCIAttrSet(handle, handle_type, const_cast<char *>(value.data()), static_cast<ub4>(value.size()),
	                    attribute, err),
	         err, what);
}
...
DirectPathLoader::DirectPathLoader(const OracleCredentials &credentials, const std::string &owner,
	...
	try {
		...
		CheckOci(Oci().OCIAttrSet(ctx_, OCI_HTYPE_DIRPATH_CTX, &rows, 0, OCI_ATTR_NUM_ROWS, err), err, "OCI_ATTR_NUM_ROWS");
		ub2 ncols = static_cast<ub2>(columns.size());
		CheckOci(Oci().OCIAttrSet(ctx_, OCI_HTYPE_DIRPATH_CTX, &ncols, 0, OCI_ATTR_NUM_COLS, err), err, "OCI_ATTR_NUM_COLS");

		OCIParam *column_list = nullptr;
		CheckOci(Oci().OCIAttrGet(ctx_, OCI_HTYPE_DIRPATH_CTX, &column_list, nullptr, OCI_ATTR_LIST_COLUMNS, err), err,
		...
		for (size_t i = 0; i < columns.size(); i++) {
			...
			CheckOci(Oci().OCIAttrSet(column, OCI_DTYPE_PARAM, &size, 0, OCI_ATTR_DATA_SIZE, err), err,
			         "OCI_ATTR_DATA_SIZE");
			if (!date_formats_[i].empty()) {
				SetTextAttr(column, OCI_DTYPE_PARAM, date_formats_[i], OCI_ATTR_DATEFORMAT, err,
				            "OCI_ATTR_DATEFORMAT");
			}
			Oci().OCIDescriptorFree(column, OCI_DTYPE_PARAM);
		}
...
void DirectPathLoader::SetCellSlow(ub4 row, ub2 col, const void *data, ub4 len, ub1 flag) {
	CheckOci(Oci().OCIDirPathColArrayEntrySet(ca_, session_.err(), row, col, static_cast<ub1 *>(const_cast<void *>(data)),
	                                    len, flag),
...
void DirectPathLoader::ConvertAndLoad(ub4 nrows) {
	...
	while (rowoff < nrows) {
		const sword rc = Oci().OCIDirPathColArrayToStream(ca_, ctx_, stream_, err, nrows, rowoff);
		if (rc == OCI_SUCCESS) {
		...
		if (rc == OCI_CONTINUE) {
			...
			ub4 converted = 0;
			CheckOci(Oci().OCIAttrGet(ca_, OCI_HTYPE_DIRPATH_COLUMN_ARRAY, &converted, nullptr, OCI_ATTR_ROW_COUNT, err), err,
			         "OCI_ATTR_ROW_COUNT");
		...
		ub4 bad_row = 0;
		ub2 bad_col = 0;
		Oci().OCIAttrGet(ca_, OCI_HTYPE_DIRPATH_COLUMN_ARRAY, &bad_row, nullptr, OCI_ATTR_ROW_COUNT, err);
		Oci().OCIAttrGet(ca_, OCI_HTYPE_DIRPATH_COLUMN_ARRAY, &bad_col, nullptr, OCI_ATTR_COL_COUNT, err);
...
void DirectPathLoader::LoadStream() {
	...
	OCIError *err = session_.err();
	const sword rc = Oci().OCIDirPathLoadStream(ctx_, stream_, err);
	...
	CheckOci(Oci().OCIDirPathStreamReset(stream_, err), err, "OCIDirPathStreamReset");
...
void DirectPathLoader::Finish() {
	...
	Flush();
	CheckOci(Oci().OCIDirPathFinish(ctx_, session_.err()), session_.err(), "OCIDirPathFinish");
...
void DirectPathLoader::Release(bool abort) {
	if (abort && ctx_ && !done_) {
		Oci().OCIDirPathAbort(ctx_, session_.err()); // status ignored: handles are freed in any case
	}
	done_ = true;
	if (ca_) {
		Oci().OCIHandleFree(ca_, OCI_HTYPE_DIRPATH_COLUMN_ARRAY);
		ca_ = nullptr;
	}
	if (stream_) {
		Oci().OCIHandleFree(stream_, OCI_HTYPE_DIRPATH_STREAM);
		stream_ = nullptr;
	}
	if (ctx_) {
		Oci().OCIHandleFree(ctx_, OCI_HTYPE_DIRPATH_CTX);
		ctx_ = nullptr;
	}
	contiguous_ = false;
	values_ = nullptr;
